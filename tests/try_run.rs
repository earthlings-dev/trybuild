...
mod tests {
  ...
  fn outcome_of<'a>(report: &'a trybuild::Report, file: &str) -> Option<&'a Result<trybuild::Outcome, trybuild::TryBuildError>> {
    report
      .cases
      .iter()
  ...
  fn verify_outcome_polarities(report: &trybuild::Report) -> Result<(), TestFailure> {
    ensure_all(&[
      (
        matches!(outcome_of(report, "compile-fail-2.rs"), Some(&Ok(trybuild::Outcome::Passed(_)))),
        "a compile_fail matching its snapshot passes",
      ),
      (
        matches!(
          outcome_of(report, "try-run-mismatch.rs"),
          Some(&Err(trybuild::TryBuildError::Diagnostics(trybuild::DiagnosticsError::Mismatch(_)))),
        ),
      ...
      (
        matches!(outcome_of(report, "run-pass-1.rs"), Some(&Ok(trybuild::Outcome::Passed(_)))),
        "a pass-test that compiles and runs cleanly passes",
      ),
      (
        matches!(
          outcome_of(report, "compile-fail-1.rs"),
          Some(&Err(trybuild::TryBuildError::Build(trybuild::BuildError::CompileFailed(_)))),
        ),
      ...
      (
        matches!(
          ...
          Some(&Err(trybuild::TryBuildError::Runner(trybuild::RunnerError::RunFailed(_)))),
        ),
  ...
  fn verify_mismatch_diff(report: &trybuild::Report) -> Result<(), TestFailure> {
    ...
    let detail = ensure_some(mismatch_detail, "the mismatch carries its diagnostic detail")?;
    ensure_all(&[
      (!detail.expected.is_empty(), "the mismatch carries the expected snapshot"),
      (
  ...
  fn try_run_reports_typed_outcomes_child() -> Result<(), TestFailure> {
    ...
    let report = ensure_ok_source(cases.try_run(trybuild::Update::Verify), "try_run sets up the throwaway project")?;
  ...
  fn try_run_reports_typed_outcomes_without_terminal_output() -> Result<(), TestFailure> {
    ...
    ensure_all(&[
      (captured.status.success(), "the typed-outcome child completed successfully"),
      (captured.stderr.is_empty(), "try_run wrote nothing to stderr"),
    ])
  }
}
