...
impl Lock {
    ...
    #[allow(
        ...
        reason = "the composite lock constructor coordinating both layers, the type's entry point to the two-layer locking abstraction this module documents"
    )]
...
impl Drop for FileLock {
    fn drop(&mut self) {
        if let &mut Self::Locked { ref path, ref done } = self {
...
#[allow(
    ...
    reason = "matching `io::ErrorKind`, which is `#[non_exhaustive]`: a catch-all arm is mandatory and the dozens of named variants carry no distinct handling here"
)]
fn create(path: &Path) -> Option<File> {
    loop {
        ...
        let future = now
            ...
            .is_some_and(|cutoff| cutoff < modified);
        if stale || future {
