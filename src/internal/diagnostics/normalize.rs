...
use self::Normalization::{
    AndOthers, AndOthersVerbose, ArrowOtherCrate, Basic, CargoRegistry, DependencyVersion,
    HeadingNote, LinesOutsideInputFile, PathDependencies, RelativeToDir, RustLib,
    StripCouldNotCompile, StripCouldNotCompile2, StripForMoreInformation, StripForMoreInformation2,
    StripLongTypeNameFiles, TrimEnd, TypeDirBackslash, Unindent, UnindentAfterHelp,
    UnindentMultilineNote, UnindentSuggestion, WorkspaceLines,
};
...
#[allow(
    ...
    reason = "the module's public normalizer entry point — also the fuzz target's and the unit tests' entry — so its call count varies by build and an #[expect] would be unfulfilled under cfg(test)"
)]
pub(in crate::internal) fn diagnostics(output: &str, context: &Context<'_>) -> Variations {
    ...
    for (variation, normalization) in result
        .variations
        .iter_mut()
        .zip(Normalization::ALL.iter().copied())
    {
...
#[allow(
    clippy::single_call_fn,
    reason = "a named per-step normalization stage, separated from the variation-set driver in `diagnostics` that maps it over every Normalization"
)]
...
impl Filter<'_> {
    ...
    #[allow(
        clippy::single_call_fn,
        reason = "the location-line normalizer, lifted out of `apply` as one named phase so the `--> ` rewrite ladder reads as a sequence of guarded steps within the nesting budget"
    )]
    fn normalize_location(&mut self, index: usize, mut line: String, indent: usize) -> String {
        ...
        if line_lower.find(&target_dir_pat) == Some(indent.saturating_add(4)) {
            ...
            if let Some(i) = line_lower.find(&workspace_pat) {
                line.replace_range(
                    i..i.saturating_add(workspace_pat.len()).saturating_sub(1),
                    "$WORKSPACE",
                );
                other_crate = true;
            }
    ...
    #[allow(
        clippy::single_call_fn,
        reason = "a named branch of `normalize_location`, isolating the source-dir vs input-file decision and its early-emit cases from the surrounding ladder"
    )]
    fn replace_source_dir(
        &self,
        line: &mut String,
        line_lower: &str,
        indent: usize,
        ...
        if self.normalization >= RelativeToDir && i == indent.saturating_add(4) {
            ...
            let after = line_lower
                .get(i.saturating_add(source_dir_pat.len())..)
                .unwrap_or("");
            ...
            line.replace_range(
                i..i.saturating_add(source_dir_pat.len()).saturating_sub(1),
                "$DIR",
            );
            if self.normalization < LinesOutsideInputFile {
                return true;
    ...
    fn replace_path_dependency(&self, line: &mut String, line_lower: &str) -> bool {
        for path_dep in self.context.path_dependencies {
            ...
            if let Some(i) = line_lower.find(&path_dep_pat) {
                ...
                line.replace_range(
                    i..i.saturating_add(path_dep_pat.len()).saturating_sub(1),
                    &var,
                );
                return true;
            }
    ...
    fn replace_cargo_registry(&self, line: &mut String, indent: usize) -> bool {
        ...
        if !line
            ...
            || !line.get(hash_end..).unwrap_or("").starts_with('/')
        {
            return false;
        }
    ...
    fn strip_line(&mut self, index: usize, mut line: String) -> Option<String> {
        ...
        if self.normalization >= AndOthersVerbose
            && self.collapse_other_types(index, &mut line, indent) == Collapse::Drop
        {
            return None;
        }
    ...
    #[allow(
        clippy::single_call_fn,
        reason = "a named branch of `strip_line`, isolating the verbose-list counter state machine and its lookahead from the guard-clause sequence"
    )]
...
#[allow(
    clippy::single_call_fn,
    reason = "a named branch of `normalize_location`, lifted out so the OUT_DIR component scan reads as one step and stays within the nesting budget"
)]
fn replace_out_dir(line: &mut String, indent: usize, target_dir_pat: &str) -> bool {
    let mut offset = indent
        .saturating_add(4)
        .saturating_add(target_dir_pat.len());
    ...
    while let Some(slash) = line.get(offset..).unwrap_or("").find('/') {
            ...
            && let Some(name) = out_dir_crate_name
        {
            ...
            line.replace_range(
                indent.saturating_add(4)..offset.saturating_add(3),
                &replacement,
            );
            return true;
        }
...
#[allow(
    clippy::single_call_fn,
    reason = "a named predicate for the <crate>-<16-hex> OUT_DIR directory shape, lifted out of replace_out_dir's component-scan loop so that loop reads as one step"
)]
...
#[allow(
    clippy::single_call_fn,
    reason = "a named predicate for the /rustc/<40-hex>/library/ standard-library path shape, keeping replace_rust_lib's three-branch $RUST ladder readable"
)]
fn is_rustc_hash_library(line: &str, indent: usize) -> bool {
    line.get(indent.saturating_add(4)..)
        .unwrap_or("")
        .starts_with("/rustc/")
        && line
...
#[allow(
    clippy::single_call_fn,
    reason = "a named predicate matching rustc's 'full type name has been written to' note that StripLongTypeNameFiles drops, kept out of strip_line's guard-clause sequence"
)]
fn is_long_type_name_note(line: &str) -> bool {
    let trimmed = line.trim_start();
    let note = trimmed.strip_prefix("= note: ").unwrap_or(trimmed);
    note.starts_with("the full type name has been written to")
        || note.starts_with("the full name for the type has been written to")
}
...
fn replace_case_insensitive(line: &str, pattern: &str, replacement: &str) -> String {
    ...
    while let Some(segment) = split.next() {
        ...
        let mut keep = line
            .get(pos..pos.saturating_add(segment.len()))
            .unwrap_or("");
        if insert_replacement {
            let end_of_maybe_path = keep.find(&[' ', ':'][..]).unwrap_or(keep.len());
            replaced.push_str(
                &keep
                    .get(..end_of_maybe_path)
                    .unwrap_or("")
                    .replace('\\', "/"),
            );
...
fn unindent(diag: String, normalization: Normalization) -> String {
    ...
    while let Some(line) = lines.next() {
        ...
        let IndentedLineKind::Code(indent) =
            indented_line_kind(next_line, false, &mut false, normalization)
        else {
            continue;
        };
        if !next_line
            .get(indent.saturating_add(1)..)
            .unwrap_or("")
            .starts_with("--> ")
        {
            continue;
...
#[allow(
    clippy::single_call_fn,
    reason = "a named phase of `unindent`, giving the block-measuring loop its own scope so its `line`/`indent` bindings don't shadow the outer pass"
)]
fn measure_unindent_block(
    first_indent: usize,
    ahead: Lines<'_>,
    normalization: Normalization,
) -> (usize, usize) {
...
fn indented_line_kind(
    ...
    spaces = spaces.saturating_add(
        line.get(spaces.saturating_add(digits)..)
            .unwrap_or("")
            .bytes()
            .take_while(is_space)
            .count(),
