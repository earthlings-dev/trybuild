...
use crate::internal::runner::{Result, RunnerError};
...
impl ExpandedTestSet {
    ...
    #[allow(
        clippy::single_call_fn,
        reason = "the accumulator constructor paired with `insert` on the same private type, keeping ExpandedTestSet's invariants self-contained"
    )]
    const fn new() -> Self {
        Self {
            vec: Vec::new(),
