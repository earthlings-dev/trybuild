...
#![allow(
    ...
    reason = "serde's `remote = \"Self\"` makes the `Dependency` derive generate inherent `serialize`/`deserialize` fns that the hand-written `Serialize`/`Deserialize` impls delegate to; same_name_method targets that derive-generated inherent impl, which cannot carry its own attribute, so it is suppressed at module scope"
)]
...
#[allow(
    clippy::single_call_fn,
    reason = "the fallible workspace-manifest reader, named to pair with get_workspace_manifest's unwrap_or_default and isolate the `?`-laden body"
)]
pub(in crate::internal) fn try_get_workspace_manifest(
    manifest_dir: &Directory,
) -> error::Result<WorkspaceManifest> {
    let cargo_toml_path = manifest_dir.join("Cargo.toml");
    let manifest_str = fs::read_to_string(cargo_toml_path).map_err(SysError::Io)?;
    let mut manifest: WorkspaceManifest =
        toml::from_str(&manifest_str).map_err(ProjectError::TomlDe)?;
...
#[allow(
    clippy::single_call_fn,
    reason = "a documented path-rewriting helper kept symmetric with fix_dependencies and fix_replacements so the three `[patch]`/`[replace]`/dep fixups read alike"
)]
...
#[allow(
    clippy::single_call_fn,
    reason = "a documented path-rewriting helper kept symmetric with fix_dependencies and fix_patches so the three `[patch]`/`[replace]`/dep fixups read alike"
)]
...
#[allow(
    ...
    reason = "serde invokes a `skip_serializing_if` predicate as `fn(&T) -> bool`; the `&bool` receiver is mandated by that call signature, not a missed by-value optimization"
)]
