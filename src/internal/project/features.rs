...
use serde::de::{self, Deserialize as _, DeserializeOwned, Deserializer};
...
#[allow(
    ...
    reason = "the infallible feature-detection entry point on this module's surface, wrapping the best-effort try_find and collapsing its error to None"
)]
...
#[allow(
    clippy::single_call_fn,
    reason = "the fallible detection routine, named and split from find so the `?`-on-Ignored body stays separate from the lossy None fallback"
)]
